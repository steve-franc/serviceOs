...
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'confirmed';

-- Set all existing public orders to confirmed (they were already processed)
-- New public orders will be 'pending' by default via the create_public_order function

-- Update create_public_order to set status to 'pending'
...
BEGIN
  ...
  INSERT INTO public.orders (
    id, staff_id, total, payment_method, notes, customer_name, customer_email,
    is_public_order, currency, restaurant_id, order_number, discount_amount, status
  ) VALUES (
    _order_id, '00000000-0000-0000-0000-000000000000', _total, _payment_method,
    NULLIF(btrim(COALESCE(_notes, '')), ''), btrim(_customer_name),
    NULLIF(btrim(COALESCE(_customer_email, '')), ''), true, 'TRY',
    _restaurant_id, _order_number, 0, 'pending'
  );
