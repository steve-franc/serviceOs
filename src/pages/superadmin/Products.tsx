    ...
    <>
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="rounded-xl bg-card border border-border shadow-sm overflow-hidden">
          <div className="hidden md:grid grid-cols-[40px_1fr_100px_140px] gap-4 px-5 py-3 border-b border-border text-xs font-medium text-muted-foreground uppercase tracking-wider">
            <span>#</span><span>Product</span><span className="text-right">Sold</span><span className="text-right">Revenue</span>
          </div>
          {isLoading ? (
            ...
            products.map((p: any, i: number) => (
              <motion.div key={p.menu_item_name} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.02, 0.5) }}
                className="grid grid-cols-1 md:grid-cols-[40px_1fr_100px_140px] gap-1 md:gap-4 items-center px-5 py-4 border-b border-border last:border-b-0 hover:bg-accent/50 transition-colors">
